// lib/recurring-service/index.ts
...
function enhanceRecurringWithMetrics(recurring: RecurringTransaction): RecurringWithNextDue {
...
async function verifyCategoryAccess(categoryId: string, userId: string): Promise<void> {
  try {
...
export async function getAllRecurring(
  ...
  const orderBy: any = { [validatedParams.sortBy]: validatedParams.sortOrder };
  ...
  logger.info("Recurring transactions fetched", { count: recurringTransactions.length, total });
...
export async function createRecurring(
  ...
  logger.info("Recurring transaction created", { id: recurring.id, name: recurring.name });
...
export async function updateRecurring(
  ...
  if (validatedData.frequency || validatedData.interval || validatedData.startDate) {
    const startDate = validatedData.startDate ? new Date(validatedData.startDate) : existing.startDate;
    const frequency = (validatedData.frequency || existing.frequency) as RecurringFrequency;
    const interval = validatedData.interval || existing.interval;
    
    updateData.nextDueDate = calculateNextDueDate(startDate, frequency, interval);
  }
...
export async function pauseRecurring(id: string, userId: string): Promise<RecurringTransaction> {
...
export async function resumeRecurring(id: string, userId: string): Promise<RecurringTransaction> {
